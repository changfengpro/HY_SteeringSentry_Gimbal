...
//单方向空气阻力模型
extern float monoDirectionalAirResistanceModel_L(float s, float v, float angle);
//完全空气阻力模型
extern float completeAirResistanceModel_L(float s, float v, float angle);
//pitch弹道补偿
extern float pitchTrajectoryCompensation_L(float s, float z, float v);
//根据最优决策得出被击打装甲板 自动解算弹道
extern void autoSolveTrajectory_L(float *pitch, float *yaw, float *aim_x, float *aim_y, float *aim_z);
